...
- [x] **初始化小程序项目**
  - [x] 使用微信开发者工具创建小程序工程
  - [x] 配置项目基本信息（appid、项目名称、项目描述）
  - [x] 规划目录结构：`pages`、`components`、`utils`、`services`、`assets` 等

- [x] **全局配置与样式**
  - [x] 配置 `app.json`（tabBar、全局路由、窗口样式）
  - [x] 配置全局主题与基础色（主色、次色、背景色、文字颜色）
  - [x] 定义全局样式文件（如 `base.wxss`：字体、按钮、卡片、栅格布局）

- [x] **网络请求封装**
  - [x] 封装通用 `request` 方法（基于 `wx.request`）
  - [x] 统一处理：baseUrl、超时、Loading、错误提示
