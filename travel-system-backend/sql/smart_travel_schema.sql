-- ============================================
-- 智能旅游系统 - 完整建库 & 建表脚本（整合版）
-- 说明：
-- 1. 直接执行本脚本即可完成库和所有表的创建
-- 2. 已合并后续变更（如 user.email、scenic_spot/food 扩展字段等）
-- ============================================
...
CREATE TABLE IF NOT EXISTS `city` (
  ...
  `image_url`   varchar(512) DEFAULT NULL COMMENT '城市图片URL',
...
-- 景点表（使用最新整合版本：含 province/city/price/image_url/free_notice 等）
...
CREATE TABLE IF NOT EXISTS `travel_route` (
  ...
  `ai_used`        tinyint(1)    NOT NULL DEFAULT 0 COMMENT '是否使用AI文案：1是 0否',
  `create_time`    datetime      DEFAULT CURRENT_TIMESTAMP,
  `update_time`    datetime      DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `create_by`      bigint(20)    DEFAULT NULL,
  `update_by`      bigint(20)    DEFAULT NULL,
